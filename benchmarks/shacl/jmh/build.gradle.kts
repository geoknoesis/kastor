...
tasks.register("shaclCompareKastorPyTierA") {
  ...
  doLast {
    ...
    val script = File(rootDir, "benchmarks/shacl/jmh/scripts/pyshacl/validate_era_style.py")
    try {
      val pyOut =
          capture(
              listOf(
      ...
      println(
          "PySHACL leg skipped (${e.javaClass.simpleName}: ${e.message}). " +
              "Set KASTOR_PYTHON or install pyshacl — see benchmarks/shacl/jmh/scripts/pyshacl/README.md",
      )
    }
  }
}

 ...
 *
 * TopBraid is manual (see `benchmarks/shacl/docs/topbraid-self-hosted.md`).
 */
fun main() {
  ...
  val pyScript = repoRoot.resolve("benchmarks/shacl/scripts/pyshacl/validate_era_style.py")

  for (base in bases) {
    val descFile = repoRoot.resolve("benchmarks/shacl/workloads/tier-a/$base.json")
    ...
    if (Files.isRegularFile(pyScript)) {
      ...
      try {
        ...
        println("Install deps: pip install -r benchmarks/shacl/scripts/pyshacl/requirements.txt")

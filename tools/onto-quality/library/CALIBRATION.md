...
| Reasoning preflight — **K07** (`HermiT preflight surfaces Kastor K07…`) | 1 | **0–1** | 0 | **1** when HermiT not supported (`assumeTrue` on classpath) |
...
**Combined outcome (latest run with `KASTOR_SKIP_EMBEDDING_TESTS` unset):** **29** passing pitfall assertions (**24** structural + **1** HermiT K07 + **4** semantic), **0** failures, **1** skip (**P09** — no fixture). With semantic tests gated off, **25** structural + K07 assertions still run; the **4** semantic rows are not executed.
...
`./gradlew :tools:onto-quality:test --tests "*OopsCalibrationTest*"` schedules **30** JUnit test invocations when embeddings are enabled: **15** original structural + **10** v0.1.x + **1** K07 HermiT + **4** semantic. With **`KASTOR_SKIP_EMBEDDING_TESTS=1`**, the nested semantic class is disabled (**26** invocations: structural + K07 HermiT).
...
- **Structural (implemented + calibrated here):** **24** OOPS **P** codes in `owl-quality-shacl.ttl` (12 legacy + 10 extension + **P01** + **P23**), plus **K01** (Kastor), across the **32** OOPS pitfall numbers that have upstream fixtures.
...
- **REASONING (v0.4):** **K07** — HermiT inconsistency preflight via `check(ontology, HERMIT)` on fixture `oops-corpus/K07.ttl` (**OopsCalibrationTest**; skipped when HermiT unavailable).
- **REASONING (backlog via triage):** P05, P28, P29, P31, P30 (mixed), P14, P15, P16-specific automation, etc.
- **OUT_OF_SCOPE:** P07, P10, P17-style judgement calls, etc.

## v0.4 — HermiT preflight (K07)

| Code | Mechanism | Corpus | Status |
|------|-----------|--------|--------|
| **K07** | `QualityChecker.check(ontology, HERMIT)` merges HermiT `ConsistencyResult` into the SHACL report | `oops-corpus/K07.ttl` | ✓ Pass when HermiT on classpath (otherwise test skipped) |

**Registry metadata:** **`QualityChecker.default()`** merges **`OOPS_PITFALL_REGISTRY`** (deactivated shapes) so **K07** findings resolve [PitfallReference.KastorExtension](src/main/kotlin/com/geoknoesis/kastor/ontoquality/QualityReport.kt) like other documented codes.
